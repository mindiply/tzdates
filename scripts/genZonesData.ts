...
async function main() {
  ...
  const timezones = tzdbData.zones
    .map(zone => zone.name)
    .sort()
    .map(
      zoneName =>
        `  ${zoneName.toUpperCase().replaceAll('/', '__').replaceAll('-', '__').replaceAll('+', '_P_')} = '${zoneName}'`
    )
    .join(',\n');
  ...
  await Promise.all([
    writeFile(
      ...
      Buffer.from(
        `export const tzdbPackedData = ${JSON.stringify(packZonesData(tzdbData))};\n`,
        'utf-8'
      )
    ),
    writeFile(
