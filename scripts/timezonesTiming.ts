...
async function main() {
  ...
  for (const unit of ['year', 'month', 'day', 'hour', 'minute', 'second']) {
    ...
    for (const testTimezone of Object.values(TimeZone)) {
      for (
        ...
        try {
          startOf(
            zdt,
            unit as 'year' | 'month' | 'day' | 'hour' | 'minute' | 'second'
          );
        } catch (err) {
          console.log(
            `Unable to do startOf of\n${JSON.stringify(zdt, null, 2)}`
          );
          console.log(err);
        }
      }
    ...
    microSecsPerIteration = ((endMs - startMs) * 1000) / i;
    console.log(
      `Completed startOf(${unit}) in ${endMs - startMs} ms after ${i} iterations, ${microSecsPerIteration} microSecs per iteration`
    );
  }
}
...
function startOf(
  ...
  switch (resolution) {
    ...
    case 'minute':
      return withZonedDateTime(dateTime, {second: 0, millisecond: 0});
    case 'hour':
      return withZonedDateTime(dateTime, {
        minute: 0,
        second: 0,
        millisecond: 0
      });
